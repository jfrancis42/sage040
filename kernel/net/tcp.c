...
static void send_data(struct tcpcb *t)
{
    ...
    for (;;) {
        ...
        if (send_range(t, t->snd_nxt, n) < 0) {
             ...
             * ARMED. rexmit_at is otherwise set only after a
             * successful send, below, and tcp_timer() skips any
             * connection whose rexmit_at is 0 -- so a failed transmit
             * would leave data queued with nothing anywhere that
             * would ever try it again.
             *
             * A transmit failure is transient: the card gets its
             * pages back as frames are drained. Treat it as a lost
             * segment, which is what it is.
             */
            if (!t->rexmit_at) {
